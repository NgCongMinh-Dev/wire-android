...
class DecryptionHandlerTest : UnitTest() {
    ...
    @Test
    fun `given backup file, userId, password, when meta data fails, then propagate meta data error error`() {
        ...
        res.onFailure {
            res.onFailure {
                (it as CryptoFailure).apply {
                    assertEquals(msg, META_DATA_ERROR_MESSAGE)
                }
            }
        }
    }
    ...
    @Test
    fun `given backup file, userId, password, when hashing fails, then propagate hash failed error error`() {
        val tempDir = createTempDir()
        val backupFile = createTextFile(tempDir)
        val password = generateText(8)
        val userId = UserId.apply()
        ...
        `when`(headerMetaData.readEncryptedMetadata(backupFile)).thenReturn(metaData)
        ...
        res.onFailure {
            res.onFailure {
                (it as CryptoFailure).apply {
                    assertEquals(msg, HASH_FAILED_ERROR_MESSAGE)
                }
            }
        }
    }
    ...
    fun `given backup file, userId, password, when meta data and hash do not match, then propagate hash error`() {
        ...
        val metaData = EncryptedBackupHeader.EMPTY
        `when`(headerMetaData.readEncryptedMetadata(backupFile)).thenReturn(EncryptedBackupHeader.EMPTY)
        `when`(crypto.hash(userId.str(), metaData.salt)).thenReturn(hash)
        ...
        res.onFailure {
            res.onFailure {
                (it as CryptoFailure).apply {
                    assertEquals(msg, HASH_NOT_MATCHING_ERROR_MESSAGE)
                }
            }
        }
    }
    ...
    fun `given backup file, userId, password, when decrypt state is null, then propagate error`() {
        ...
        `when`(headerMetaData.readEncryptedMetadata(backupFile)).thenReturn(metaData)
        `when`(crypto.hash(any(), any())).thenReturn(hash)
        ...
        res.onFailure {
            res.onFailure {
                (it as CryptoFailure).apply {
                    assertEquals(msg, DECRYPT_INIT_ERROR_MESSAGE)
                }
            }
        }
    }
    ...
    fun `given backup file, userId, password, when decrypted message part is invalid, then propagate error`() {
        ...
        `when`(headerMetaData.readEncryptedMetadata(backupFile)).thenReturn(metaData)
        `when`(crypto.hash(any(), any())).thenReturn(hash)
        `when`(crypto.initDecryptState(hash, header)).thenReturn(hash)
        ...
        res.onFailure {
            res.onFailure {
                (it as CryptoFailure).apply {
                    assertEquals(msg, BACKUP_FAIURE_ERROR_MESSAGE)
                }
            }
        }
    ...
    companion object {
        ...
        private const val HASH_NOT_MATCHING_ERROR_MESSAGE = "Uuid hashes don't match"
        private const val HASH_FAILED_ERROR_MESSAGE = "Uuid hashing failed"
        private const val META_DATA_ERROR_MESSAGE = "Metadata could not be read"
        private const val DECRYPT_INIT_ERROR_MESSAGE = "Failed to init decrypt"
        private const val BACKUP_FAIURE_ERROR_MESSAGE = "Failed to decrypt backup, got code 1"
